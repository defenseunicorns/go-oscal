...
UNDOCTORED_SCHEMA ?= testdata/doctor/oscal_complete_schema-1-1-2.json
...
generate-latest-schema: clean build
	$(BINDIR)/$(BINNAME) generate -f $(OSCAL_LATEST_SCHEMA) -x tmp/oscal_complete_schema.xsd --pkg $(OSCAL_LATEST_PACKAGE) --tags json,yaml -o $(OSCAL_LATEST_OUTPUT)/types.go