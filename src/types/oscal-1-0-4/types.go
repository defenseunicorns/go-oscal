...
type Catalog struct {
	...
	// A globally unique identifier with cross-instance scope for this catalog
...
type ComponentDefinition struct {
	...
	Metadata                   Metadata                     `json:"metadata" yaml:"metadata"`
	// A machine-oriented, globally unique identifier with cross-instance scope
	// that can be used to reference this component definition elsewhere in this or
	// other OSCAL instances. The locally defined UUID of the component definition
...
type Profile struct {
	...
	// A machine-oriented, globally unique identifier with cross-instance scope
	// that can be used to reference this profile elsewhere in this or other OSCAL
	// instances. The locally defined UUID of the profile can be used to reference
	// the data item locally or globally (e.g., in an imported OSCAL instance).This
	// identifier should be assigned per-subject, which means it should be
	// consistently used to identify the same profile across revisions of the
	// document.
	UUID string `json:"uuid" yaml:"uuid"`
}
...
// Provides information about the publication and availability of the containing
// document.
...
// A structured information object representing a security or privacy control.
// Each security or privacy control within the Catalog is defined by a distinct
// control instance.
type Control struct {
	// A textual label that provides a sub-type or characterization of the control.
	Class    string     `json:"class,omitempty" yaml:"class,omitempty"`
	Controls *[]Control `json:"controls,omitempty" yaml:"controls,omitempty"`
	// A human-oriented, locally unique identifier with instance scope that can be
	// used to reference this control elsewhere in this and other OSCAL instances
	// (e.g., profiles). This id should be assigned per-subject, which means it
	// should be consistently used to identify the same control across revisions of
	// the document.
...
type Group struct {
	...
	Controls *[]Control `json:"controls,omitempty" yaml:"controls,omitempty"`
	Groups   *[]Group   `json:"groups,omitempty" yaml:"groups,omitempty"`
	// A human-oriented, locally unique identifier with cross-instance scope that
	// can be used to reference this defined group elsewhere in in this and other
	// OSCAL instances (e.g., profiles). This id should be assigned per-subject,
	// which means it should be consistently used to identify the same group across
	// revisions of the document.
...
type Parameter struct {
	...
	// A human-oriented, locally unique identifier with cross-instance scope that
	// can be used to reference this defined parameter elsewhere in this or other
	// OSCAL instances. When referenced from another OSCAL instance, this
	// identifier must be referenced in the context of the containing resource
	// (e.g., import-profile). This id should be assigned per-subject, which means
	// it should be consistently used to identify the same subject across revisions
	// of the document.
	ID string `json:"id" yaml:"id"`
...
type Capability struct {
	...
	Remarks string      `json:"remarks,omitempty" yaml:"remarks,omitempty"`
	// A machine-oriented, globally unique identifier with cross-instance scope
	// that can be used to reference this capability elsewhere in this or other
	// OSCAL instances. The locally defined UUID of the capability can be used to
	// reference the data item locally or globally (e.g., in an imported OSCAL
	// instance).This UUID should be assigned per-subject, which means it should be
...
// The import designates a catalog or profile to be included (referenced and
// potentially modified) by this profile. The import also identifies which
// controls to select using the include-all, include-controls, and
...
// A Merge element provides structuring directives that drive how controls are
// organized after resolution.
type Merge struct {
	// An As-is element indicates that the controls should be structured in
	// resolution as they are structured in their source catalogs. It does not
	// contain any elements or attributes.
	AsIs bool `json:"as-is,omitempty" yaml:"as-is,omitempty"`
	// A Combine element defines how to combine multiple (competing) versions of
	// the same control.
...
// as a namespace qualified name/value pair. The value of a property is a simple
...
type Property struct {
	...
	// property's name. This can be used to further distinguish or discriminate
	// between the semantics of multiple properties of the same object with the
	// same name and ns.
	Class string `json:"class,omitempty" yaml:"class,omitempty"`
	...
	Remarks string `json:"remarks,omitempty" yaml:"remarks,omitempty"`
	// A machine-oriented, globally unique identifier with cross-instance scope
	// that can be used to reference this defined property elsewhere in this or
	// other OSCAL instances. This UUID should be assigned per-subject, which means
	// it should be consistently used to identify the same subject across revisions
	// of the document.
	UUID string `json:"uuid,omitempty" yaml:"uuid,omitempty"`
...
// A resource associated with content in the containing document. A resource may
// be directly included in the document base64 encoded or may point to one or
// more equivalent internet resources.
type Resource struct {
	// The Base64 alphabet in RFC 2045 - aligned with XSD.
	Base64 *Base64 `json:"base64,omitempty" yaml:"base64,omitempty"`
	// A citation consisting of end note text and optional structured bibliographic
	// data.
	...
	Title string `json:"title,omitempty" yaml:"title,omitempty"`
	// A machine-oriented, globally unique identifier with cross-instance scope
	// that can be used to reference this defined resource elsewhere in this or
	// other OSCAL instances. This UUID should be assigned per-subject, which means
	// it should be consistently used to identify the same subject across revisions
	// of the document.
	UUID string `json:"uuid" yaml:"uuid"`
...
type LocalObjective struct {
	// A human-oriented identifier reference to a control with a corresponding id
	// value. When referencing an externally defined control, the Control
	// Identifier Reference must be used in the context of the external / imported
	// OSCAL instance (e.g., uri-reference).
...
// A document identifier qualified by an identifier scheme. A document
// identifier provides a globally unique identifier with a cross-instance scope
// that is used for a group of documents that are to be treated as different
// versions of the same document. If this element does not appear, or if the
// value of this element is empty, the value of "document-id" is equal to the
// value of the "uuid" flag of the top-level root element.
...
type Location struct {
	...
	// A machine-oriented, globally unique identifier with cross-instance scope
	// that can be used to reference this defined location elsewhere in this or
	// other OSCAL instances. The locally defined UUID of the location can be used
	// to reference the data item locally or globally (e.g., from an importing
	// OSCAL instance). This UUID should be assigned per-subject, which means it
	// should be consistently used to identify the same subject across revisions of
	// the document.
	UUID string `json:"uuid" yaml:"uuid"`
...
type Party struct {
	...
	Type string `json:"type" yaml:"type"`
	// A machine-oriented, globally unique identifier with cross-instance scope
	// that can be used to reference this defined party elsewhere in this or other
	// OSCAL instances. The locally defined UUID of the party can be used to
	// reference the data item locally or globally (e.g., from an importing OSCAL
	// instance). This UUID should be assigned per-subject, which means it should
	// be consistently used to identify the same subject across revisions of the
	// document.
	UUID string `json:"uuid" yaml:"uuid"`
...
// An entry in a sequential list of revisions to the containing document in
// reverse chronological order (i.e., most recent previous revision first).
...
// Defines a function assumed or expected to be assumed by a party in a specific
// situation.
type Role struct {
	// A summary of the role's purpose and associated responsibilities.
	Description string `json:"description,omitempty" yaml:"description,omitempty"`
	// A human-oriented, locally unique identifier with cross-instance scope that
	// can be used to reference this defined role elsewhere in this or other OSCAL
	// instances. When referenced from another OSCAL instance, the locally defined
	// ID of the Role from the imported OSCAL instance must be referenced in the
	// context of the containing resource (e.g., import,
	// import-component-definition, import-profile, import-ssp or import-ap). This
	// ID should be assigned per-subject, which means it should be consistently
	// used to identify the same subject across revisions of the document.
	ID      string      `json:"id" yaml:"id"`
	Links   *[]Link     `json:"links,omitempty" yaml:"links,omitempty"`
	Props   *[]Property `json:"props,omitempty" yaml:"props,omitempty"`
...
type AssessedControls struct {
	...
	Description     string           `json:"description,omitempty" yaml:"description,omitempty"`
	ExcludeControls *[]SelectControl `json:"exclude-controls,omitempty" yaml:"exclude-controls,omitempty"`
	IncludeAll      *IncludeAll      `json:"include-all,omitempty" yaml:"include-all,omitempty"`
	IncludeControls *[]SelectControl `json:"include-controls,omitempty" yaml:"include-controls,omitempty"`
	Links           *[]Link          `json:"links,omitempty" yaml:"links,omitempty"`
	Props           *[]Property      `json:"props,omitempty" yaml:"props,omitempty"`
	Remarks         string           `json:"remarks,omitempty" yaml:"remarks,omitempty"`
}
...
type ResponsibleRole struct {
	...
	Props      *[]Property `json:"props,omitempty" yaml:"props,omitempty"`
	Remarks    string      `json:"remarks,omitempty" yaml:"remarks,omitempty"`
	// A human-oriented identifier reference to roles responsible for the business
	// function.
	RoleId string `json:"role-id" yaml:"role-id"`
}
...
type Part struct {
	...
	Class string `json:"class,omitempty" yaml:"class,omitempty"`
	// A human-oriented, locally unique identifier with cross-instance scope that
	// can be used to reference this defined part elsewhere in this or other OSCAL
	// instances. When referenced from another OSCAL instance, this identifier must
	// be referenced in the context of the containing resource (e.g.,
	// import-profile). This id should be assigned per-subject, which means it
	// should be consistently used to identify the same subject across revisions of
	// the document.
	ID    string  `json:"id,omitempty" yaml:"id,omitempty"`
	Links *[]Link `json:"links,omitempty" yaml:"links,omitempty"`
...
type ControlImplementationSet struct {
	...
	// A machine-oriented, globally unique identifier with cross-instance scope
	// that can be used to reference a set of implemented controls elsewhere in
	// this or other OSCAL instances. The locally defined UUID of the control
	// implementation set can be used to reference the data item locally or
...
// Relates the poam-item to a set of referenced observations that were used to
...
// A Combine element defines how to combine multiple (competing) versions of the
// same control.
...
type CustomGrouping struct {
	Groups         *[]ControlGroup   `json:"groups,omitempty" yaml:"groups,omitempty"`
...
// An Alter element specifies changes to be made to an included control when a
// profile is resolved.
type Alter struct {
...
// A pointer to an external resource with an optional hash for verification and
// change detection.
...
type ImplementedRequirementControlImplementation struct {
	...
	// OSCAL instance (e.g., uri-reference).
	ControlId string `json:"control-id" yaml:"control-id"`
	// A suggestion for how the specified control may be implemented if the
	// containing component or capability is instantiated in a system security
	// plan.
	Description      string                            `json:"description" yaml:"description"`
	Links            *[]Link                           `json:"links,omitempty" yaml:"links,omitempty"`
	Props            *[]Property                       `json:"props,omitempty" yaml:"props,omitempty"`
	...
	// A machine-oriented, globally unique identifier with cross-instance scope
	// that can be used to reference a specific control implementation elsewhere in
	// this or other OSCAL instances. The locally defined UUID of the control
...
type ControlGroup struct {
	...
	Groups *[]ControlGroup `json:"groups,omitempty" yaml:"groups,omitempty"`
	// A human-oriented, locally unique identifier with cross-instance scope that
	// can be used to reference this defined group elsewhere in this or other OSCAL
	// instances. When referenced from another OSCAL instance, this identifier must
	// be referenced in the context of the containing resource (e.g.,
	// import-profile). This id should be assigned per-subject, which means it
	// should be consistently used to identify the same group across revisions of
	// the document.
	ID             string            `json:"id,omitempty" yaml:"id,omitempty"`
	InsertControls *[]InsertControls `json:"insert-controls,omitempty" yaml:"insert-controls,omitempty"`
	Links          *[]Link           `json:"links,omitempty" yaml:"links,omitempty"`
...
type Remove struct {
	...
	ById string `json:"by-id,omitempty" yaml:"by-id,omitempty"`
	// Identify items to remove by the name of the item's information element name,
	// e.g. title or prop
...
type InformationType struct {
	// The expected level of impact resulting from the disruption of access to or
	// use of the described information or the information system.
	...
	// The expected level of impact resulting from the unauthorized disclosure of
	...
	Description string `json:"description" yaml:"description"`
	// The expected level of impact resulting from the unauthorized modification of
	// the described information.
